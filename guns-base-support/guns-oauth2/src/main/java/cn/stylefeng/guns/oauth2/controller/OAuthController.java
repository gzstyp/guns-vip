...
public class OAuthController extends BaseController {
    ...
    public String callback(@PathVariable("source") String source, AuthCallback callback) {
        ...
        final AuthRequest authRequest = OAuthRequestFactory.getAuthRequest(source);
        final AuthResponse<AuthUser> authResponse = authRequest.login(callback);
        final AuthUser oauthUser = authResponse.getData();
    ...
    public String avatar() {
        final Long userId = LoginContextHolder.getContext().getUser().getId();
        //获取第三方登录用户的头像
        final String avatarUrl = oauthUserInfoService.getAvatarUrl(userId);
